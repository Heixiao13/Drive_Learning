...
static ssize_t globalfifo_read(struct file* filp,
    ...
    DECLARE_WAITQUEUE(
        wait,
        current);  // 定义当前进程的等待队列wait，current指针指向当前在运行的进程

    down(&dev->sem); /*获得信号量*/
    add_wait_queue(&dev->r_wait, &wait);
        /*加入读等待队列头 到内核*/  // 把wait添加到等待队列头r_wait指向的等待队列链表中，并不代表已经睡眠了，还需要调度函数的调度
    ...
    if (dev->current_len == 0) {
        ...
        schedule();
            /*调度其他进程执行*/  // 此时读进程才会真正的睡眠，直至被写进程唤醒。在睡眠途中，如果用户给读进程发送了信号，那么也会唤醒睡眠的进程
        if (signal_pending(current)) {
            /*如果是因为信号唤醒*/  // 因为是调度出去，进程状态是浅度睡眠，唤醒它的有可能是信号
            ret =
                -ERESTARTSYS;  // 表示信号函数处理完毕后重新执行信号处理函数前的某个系统调用
            goto out2;
        }
    ...
    if (copy_to_user(
            buf, dev->mem,
            count)) {  // 参数：to from
                       // count，成功返回0，失败是返回还没有拷贝到用户空间的字节数
        ret = -EFAULT;
        goto out;
    } else {
        memcpy(dev->mem, dev->mem + count, dev->current_len - count);
            /*数据前移*/  // memcpy将(dev->mem + count)开始的(dev->current_len -
                          // count)字节的数据移动到缓冲区最开始的地方
        dev->current_len -= count; /*有效数据长度减少*/
        printk(KERN_INFO "read %ld bytes(s),current_len:%d\n", count,
               dev->current_len);

        wake_up_interruptible(&dev->w_wait);
            /*唤醒写等待队列*/  // 已经读完数据，就要唤醒写队列来进行写数据
        ret = count;
    }
out:
    up(&dev->sem); /*释放信号量*/
...
static ssize_t globalfifo_write(struct file* filp,
    ...
    if (count >
        GLOBALFIFO_SIZE -
            dev->current_len) {  // 如果fifo的大小大于有效内存的长度，则下次再写
        /*如果要拷贝的数据大于 剩余有效内存长度
         *则 只拷贝最大 能装下的长度*/
        count =
            GLOBALFIFO_SIZE - dev->current_len;  // count保留下次再写的数据大小
    }
    if (copy_from_user(
            dev->mem + dev->current_len, buf,
            count)) {  // 参数to，from，count，to：(dev->mem) +
                       // (dev->current_len)，因为写入数据了，所以当前的共享数据区，要移位
        ret = -EFAULT;
        goto out;
    } else {
